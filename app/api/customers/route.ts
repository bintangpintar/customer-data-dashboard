...
// Mock CSV data (fallback for initial load)
...
function mapSupabaseToCustomer(data: any): Customer {
  // Map Supabase record to Customer type
  return {
    nama: data.name || '',
    noHp: data.phone || '',
    creditScore: data.credit_score || 70,
    typeCollateral: data.collateral_type || 'Elektronik',
    highestLoan: data.highest_loan || 0,
    percentileLoan: data.percentile_loan || 'bottom 75%',
    highValue: data.high_value || false,
    vip: data.vip || false,
  };
}
...
export async function GET(request: NextRequest) {
  try {
    ...
    const supabaseCustomers = await getCustomers();
    
    if (supabaseCustomers && supabaseCustomers.length > 0) {
      const mapped = supabaseCustomers.map(mapSupabaseToCustomer);
    ...
    return NextResponse.json({ data: [] }, { status: 200 }); // Return empty array on error
  }
}
